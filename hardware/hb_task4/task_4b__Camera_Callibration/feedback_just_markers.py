...
class camera_node(Node):
    def __init__(self):
        ...
        self.image_subscription = self.create_subscription(Image, '/image_rect', self.image_callback, 10)
        ...
        self.MARKER_RGB_IMAGE= np.zeros((500, 500), dtype=np.uint8)
    ...
    def aruco_detection(self, frame):
        ...
        if np.any(np.equal(self.arena_corners, None)):
            ...
            # self.image_publisher.publish(self.cv_bridge.cv2_to_imgmsg(cv_image, 'passthrough'))
        ...
        if debug:
            #BOT 1
            self.MARKER_RGB_IMAGE[self.bot1_loc[-1][1]][self.bot1_loc[-1][0]] = 255
            #BOT 2
            self.MARKER_RGB_IMAGE[self.bot2_loc[-1][1]][self.bot2_loc[-1][0]] = 255
            #BOT 3
            self.MARKER_RGB_IMAGE[self.bot3_loc[-1][1]][self.bot3_loc[-1][0]] = 255

            cv.imshow("Tracker", self.MARKER_RGB_IMAGE)
            cv.waitKey(1)
